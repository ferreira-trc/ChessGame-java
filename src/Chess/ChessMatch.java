...
public class ChessMatch {
    ...
    public ChessPiece perfomChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
        ...
        if (movedPiece instanceof Pawn) {
            boolean promotionConditionWhitePawn = movedPiece.getColor() == Color.WHITE && target.getRow() == 0;
            boolean promotionConditionBlackPawn = movedPiece.getColor() == Color.BLACK && target.getRow() == 7;

            if (promotionConditionWhitePawn || promotionConditionBlackPawn) {
    ...
    private Piece makeMove(Position source, Position target) {
        ...
        // Sepecial move castling kingside rook
        ...
        // Sepecial move castling queenside rook
        if (p instanceof King && target.getCol() == source.getCol() - 2) {
        ...
        if (p instanceof Pawn) {
            if (source.getCol() != target.getCol() && capturedPiece == null) {
                ...
                if (p.getColor() == Color.WHITE) {
                    pawPosition = new Position(target.getRow() + 1, target.getCol());
                } else {
                    pawPosition = new Position(target.getRow() - 1, target.getCol());
                }
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece) {
        ...
        if (p instanceof Pawn) {
            if (source.getCol() != target.getCol() && capturedPiece == enPassantVulnerable) {
                ...
                if (p.getColor() == Color.WHITE) {
                    pawPosition = new Position(3, target.getCol());
                } else {
                    pawPosition = new Position(4, target.getCol());
                }
